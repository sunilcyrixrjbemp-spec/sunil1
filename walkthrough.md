...
*   **Frontend Client-side Generation ([MonthSummaryPage.tsx](file:///c:/Users/Cyrix%20HealthCare/Desktop/Sunil%20React.tsx/frontend/src/pages/MonthSummaryPage.tsx))**:
    ...
    *   Added a `waitForImages` helper function to append elements off-screen into the active DOM flow, waiting until all attachment bill images are fully downloaded and rendered by the browser before generating the canvas. This guarantees perfect formatting and ensures that all bill receipts appear in both single and bulk PDF ZIP files.
    *   Corrected the SQL queries inside `handleGetEngineerAdvance`, `handleSaveEngineerAdvance`, and consolidated reports fetching to query `user_id` instead of the non-existent `user_code` column on the `engineer_advances` D1 table, fixing the SQLite SQLITE_ERROR/D1_ERROR during single PDF downloads.
    *   Set the PDF generator configuration options to `orientation: 'landscape'` and specified a fixed element width of `1120px` to match the exact aspect ratio and design of the landscape Cyrix expenses reimbursement form.
    *   Enhanced `getAbsoluteUrl` to resolve relative upload paths using the backend API endpoint (`VITE_API_URL`), ensuring attachment images fetch correctly regardless of hosting setup.
    *   Implemented a nested container wrapper (`wrapper` + `inner` div) for both single and bulk PDF generation, which keeps the element hidden offscreen from the user while allowing `html2pdf` to clone and render the child element with standard static coordinates. This prevents the canvas drawer from rendering a blank page.
