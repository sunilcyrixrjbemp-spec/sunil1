...
export default function AdminPage() {
  ...
  const [activeTab, setActiveTab] = useState<"users" | "approvals" | "settings">((() => {
    const saved = localStorage.getItem("admin_active_tab");
    if (saved === "users" || saved === "approvals" || saved === "settings") {
      return saved;
    }
    return "users";
  }));

  const handleTabChange = (tab: "users" | "approvals" | "settings") => {
    setActiveTab(tab);
