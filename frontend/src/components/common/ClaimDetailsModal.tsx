    ...
    <div className="rounded-lg border border-slate-200 bg-white p-2.5 shadow-2xs space-y-2">
      ...
      {hasLegDeduction && (
        <div className="bg-rose-50/90 p-2.5 rounded-lg border border-rose-200 space-y-1.5 text-[9.5px]">
          ...
          {/* ── 1-LINE POLICY DEDUCTION SUMMARY ── */}
          {(baseLocationDeductionReason || (isInDistrictLeg && (isTaEdited || isDaEdited || daAmt === 0)) || isKmEdited || kmDeductionReason || (isTaEdited && !isInDistrictLeg) || daDeductionReason || (isDaEdited && !isInDistrictLeg && !baseLocationDeductionReason)) && (
            <div className="text-[9.5px] font-medium text-slate-700 bg-rose-50 border border-rose-200 rounded px-2 py-1.5 flex items-center gap-1.5 whitespace-nowrap overflow-x-auto">

              {/* Label */}
              <span className="shrink-0 font-extrabold text-rose-700">⚠️ Policy:</span>
                ...
                <>
                  ...
                  {isTaEdited && (
                    <>
                      ...
                      <span className="shrink-0 bg-rose-100 text-rose-700 font-extrabold px-1.5 py-0.5 rounded border border-rose-200">{rupee(estimatedSubmittedTa)} → {rupee(taAmt)}</span>
                  ...
                  {(isDaEdited || daAmt === 0) && estimatedSubmittedDa > 0 && (
                    <>
                      ...
                      <span className="shrink-0 bg-rose-100 text-rose-700 font-extrabold px-1.5 py-0.5 rounded border border-rose-200">{rupee(estimatedSubmittedDa)} → {rupee(daAmt)}</span>
              ...
              {(isKmEdited || kmDeductionReason || (isTaEdited && !isInDistrictLeg)) && (
                <>
                  {(baseLocationDeductionReason || (isInDistrictLeg && (isTaEdited || isDaEdited || daAmt === 0))) && <span className="shrink-0 text-slate-300 font-bold">·</span>}
                  <span className="shrink-0 bg-amber-100 text-amber-800 font-bold px-1.5 py-0.5 rounded border border-amber-200">
                    ⚙️ {isKmEdited ? `KM: ${origKm}→${km} km` : `Rate: ${km} km @ ₹${ratePerKm}/km`}
                  </span>
                  <span className="shrink-0 text-slate-500">—</span>
                  <span className="shrink-0">TA:</span>
                  <span className="shrink-0 bg-rose-100 text-rose-700 font-extrabold px-1.5 py-0.5 rounded border border-rose-200">{rupee(estimatedSubmittedTa)} → {rupee(taAmt)}</span>
              ...
              {(daDeductionReason || (isDaEdited && !isInDistrictLeg && !baseLocationDeductionReason)) && (
                <>
                  ...
                  <span className="shrink-0 bg-amber-100 text-amber-800 font-bold px-1.5 py-0.5 rounded border border-amber-200">⚙️ DA Grade Cap</span>
                  <span className="shrink-0 text-slate-500">—</span>
                  <span className="shrink-0">DA:</span>
                  <span className="shrink-0 bg-rose-100 text-rose-700 font-extrabold px-1.5 py-0.5 rounded border border-rose-200">{rupee(estimatedSubmittedDa)} → {rupee(daAmt)}</span>
                  <span className="shrink-0 text-rose-600 font-bold">(-{rupee(estimatedSubmittedDa - daAmt)})</span>
              ...
              {calculatedDeduction > 0 && (
                <>
                  <span className="shrink-0 text-slate-300 font-bold">|</span>
                  <span className="shrink-0 bg-rose-600 text-white font-extrabold px-2 py-0.5 rounded">Total Deducted: -{rupee(calculatedDeduction)}</span>
                </>
              )}
            </div>
          )}
