...
async function applyItineraryEditsAndLog(env, expense, itineraryEdits, currentUser, comments) {
  ...
  const legsRows = await env.DB.prepare(
    "SELECT * FROM expense_itineraries WHERE exp_id = ? OR exp_id = ? OR exp_id = ? OR LOWER(exp_id) = LOWER(?) OR exp_id LIKE '%' || ?"
  ).bind(appExpCodeStr, appExpIdStr, appPkPaddedCode, appExpCodeStr, appExpIdStr).all();
