...
export const expenseService = {
  ...
  checkComplaintId: async (complaintId: string): Promise<any> => {
    const response = await api.get(`/expense/calls/open-calls?complaint_id=${encodeURIComponent(complaintId)}`);
    return response.data;
  },
