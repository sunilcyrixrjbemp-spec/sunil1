...
export async function handleGetExpenseDetails(request, env, params, query, user) {
  ...
  const [
    ...
    env.DB.prepare("SELECT * FROM expense_itineraries WHERE exp_id = ? OR exp_id = ? OR exp_id = ? OR LOWER(exp_id) = LOWER(?) OR exp_id LIKE '%' || ? ORDER BY leg_number ASC").bind(expCodeStr, expIdStr, pkPaddedCode, expCodeStr, expIdStr).all().catch(() => ({ results: [] })),
    env.DB.prepare("SELECT * FROM expense_attachments WHERE exp_id = ? OR exp_id = ? OR exp_id = ? OR LOWER(exp_id) = LOWER(?) OR exp_id LIKE '%' || ?").bind(expCodeStr, expIdStr, pkPaddedCode, expCodeStr, expIdStr).all().catch(() => ({ results: [] })),
